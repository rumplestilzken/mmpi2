...
public class ScaleProcessor {
    ...
    public void processCritical(List<QuestionData.QuestionAnswerData> answers, List<CriticalScale> criticalScales) {
        criticalScales.forEach(currentScale -> {
            List<String> trueValues = currentScale.getTrueQuestions().stream().filter(currentQuestion -> answerMap.get(Integer.parseInt(currentQuestion)) == Boolean.TRUE).toList();
            List<String> falseValues = currentScale.getFalseQuestions().stream().filter(currentQuestion -> answerMap.get(Integer.parseInt(currentQuestion)) == Boolean.FALSE).toList();
...
//            System.out.println("Index1:" + currentPair.index1 + ",Answer1:" + answer1 + ",Index2:" + currentPair.getIndex2() + ",Answer2:" + answer2);
    ...
    private void processBooleanScale(Class clazz, Boolean bool, List<QuestionData.QuestionAnswerData> answers, List<Scale> scaleList) {
        scaleList.stream().filter(clazz::isInstance).findFirst().get().rawScore = answers.stream().filter(i -> i.getAnswer() == bool).count();
    }
}
